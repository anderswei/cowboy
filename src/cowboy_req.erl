...
do_reply(Status, Headers, _, Req=#{method := <<"HEAD">>}) ->
	ResponseHeaders = response_headers(Headers, Req),
	{NewStatus, NewHeaders, NewReq} = do_hook(Status, ResponseHeaders, Req),
	cast({response, NewStatus, NewHeaders, <<>>}, NewReq),
	done_replying(NewReq, true);
do_reply(Status, Headers, Body, Req) ->
	ResponseHeaders = response_headers(Headers, Req),
	{NewStatus, NewHeaders, NewReq} = do_hook(Status, ResponseHeaders, Req),
	cast({response, NewStatus, NewHeaders, Body}, NewReq),
	done_replying(NewReq, true).
...
do_hook(Status, Headers, Req) ->
	...
	case erlang:function_exported(Handler, response_hook, 3) of
		true ->
			try
				Handler:response_hook(Status, Headers, Req) of
				{ok, NewStatus, NewHeaders, NewReq} ->
					{NewStatus, NewHeaders, NewReq};
				_ ->
					{Status, Headers, Req}
			catch _:_ ->
				{Status, Headers, Req}
